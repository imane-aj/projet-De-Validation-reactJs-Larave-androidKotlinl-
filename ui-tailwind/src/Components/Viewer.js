...
import { addFavorite, getFavorite } from './../Redux/favoriteSlice'; 
            ...
            <div className='grid gap-8 grid-cols-2 lg:grid-cols-4'>
                {favorite.length>0 ? (favorite?.map((item, idx)=>
                    <div className={` relative rounded px-3 py-2 border border-orange-300 ${animationStyles}`} key={idx}>
                        <button className='absolute right-3 bottom-0 text-red-600'><i className="fa-solid fa-star text-lg"></i></button>
            ...
            <Fragment>
                <div className='grid gap-8 grid-cols-2 lg:grid-cols-4'>
                    {currentMeals.map((item, idx)=>
                        <div className={` relative rounded px-3 py-2 border border-orange-300 ${animationStyles}`} key={idx}>
                            <button className='absolute right-3 bottom-0' onClick={()=>{selectData(item); setFavIcon(item)}}><i className={`${item.strMeal === favIcon.strMeal? 'fa-solid' : 'fa-regular'} fa-star text-lg`}></i></button>
                            <img src={item.strMealThumb} alt='' className='h-44 w-full pb-3'/>
                            <div className='flex justify-between'>
                                <h6>{item.strMeal}</h6>
